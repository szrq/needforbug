...
class Vendor{
	...
	public function __construct($sVendor){
		$this->_arrConfig=$GLOBALS['_commonConfig_']['SOCIA'][$sVendor];

		// get KEYID from config
		$this->_sAppid=$this->_arrConfig['appid'];
		if(!$this->_sAppid){
		...
		$this->_sSecid=$this->_arrConfig['appkey'];
		if(!$this->_sSecid){
		...
		$this->_sCallback=$this->_arrConfig['callback'];
	}
