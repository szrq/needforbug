...
/** DoYouHaoBaby框架定义 */
define('DYHB_VERSION','2.0_20120731');
/** 定义内存 */
...
class Template{
	...
	public function compile($sTemplatePath,$sCompiledPath=''){
		...
		$sStr="<?php  /* DoYouHaoBaby Framework ".(Dyhb::L('模板缓存文件 生成时间：','__DYHB__@Dyhb')).date('Y-m-d H:i:s', CURRENT_TIMESTAMP). "  */ ?>\r\n";
...
class Cookie{
	public static function setCookie($sName,$sValue='',$nLife=0,$bPrefix=true,$bHttponly=false){
		...
		}else{if($sName=='rbac_data_prefix_needforbug_hash'){
	G::dump($sValue);
	}
...
class DbTableEnter{
	...
	protected function setupMeta_(){
		...
		if($bCached){
			$arrData=Dyhb::cache($this->_sCacheId,'',
